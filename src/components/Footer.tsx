...
export default function Footer({ settings }: FooterProps) {
  const { instagramUrl, facebookUrl, linkedinUrl, contactEmail } = settings;
  return (
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div>
          ...
          <ul className="space-y-3.5 text-sm text-gray-300">
            ...
            {/* 2. OFFICIAL EMAIL - now CMS-driven via siteSettings.contactEmail */}
            {contactEmail && (
              <li className="flex items-center gap-3 group">
                ...
                <a href={`mailto:${contactEmail}`} className="group-hover:text-white transition-colors truncate" title="Send official email">
                  {contactEmail}
                </a>
              </li>
            )}
            
            {/* 3. SOCIAL MEDIA - now CMS-driven via siteSettings, only renders if set */}
            {(instagramUrl || facebookUrl || linkedinUrl) && (
              <li className="flex items-center gap-4 pt-3 border-t border-white/10">
                {instagramUrl && (
                  <a href={instagramUrl} target="_blank" rel="noopener noreferrer" className="hover:text-wie-light transition-colors" title="Follow UNSB on Instagram">
                ...
                {facebookUrl && (
                  <a href={facebookUrl} target="_blank" rel="noopener noreferrer" className="hover:text-wie-light transition-colors" title="Follow UNSB on Facebook">
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z"/></svg>
                  </a>
                )}
                {linkedinUrl && (
                  <a href={linkedinUrl} target="_blank" rel="noopener noreferrer" className="hover:text-wie-light transition-colors" title="Follow UNSB on LinkedIn">
